...
val spark = SparkSession.builder().appName("pca_project").getOrCreate()

// Use Spark to read in the Cancer_Data file.
val dataset = spark.read.option("header", "true").option(
  "inferSchema", "true").format("csv").load("Cancer_Data.csv")
...
val assembler_output = assembler.transform(dataset).select($"features")
...
val scaler_model = scaler.fit(assembler_output)

// Normalize each feature to have unit standard deviation.
// Use transform() off of this scalerModel object to create your scaledData
val scaled_data = scaler_model.transform(assembler_output)
...
val pca_df = pca.transform(scaled_data)

// Show the new pcaFeatures
val result = pca_df.select("pca_features")
