...
val df_formatted = df.select(df("Clicked on Ad").as("label"),
  hour(df("Timestamp")).as("Hour"),
...
val Array(training, test) = df_formatted.randomSplit(
  Array(train_ratio, 1 - train_ratio), seed=random_seed)
